...
class User(BaseModel, db.Model):
    ...
    # 给函数加上property装饰器，函数变为属性，属性名即为函数名
    @property
    def password(self):
        """
        读取属性的函数行为 print(user.password)
        """
        # 读取属性行为，必须要有返回值，函数返回值作为属性值
        # return "xxxx"  因为该处返回值没有意义，可以直接抛出异常，提示不能读取操作
        raise AttributeError("目前该属性不能读取，只能设置")

    # 使用password.setter，对应设置属性操作
    @password.setter
    def password(self, value):
        """
        设置属性 user.password = "xxxxx"
        :param value: 设置属性时的数据， value="xxxxx"
        :return:
        """
        self.password_hash = generate_password_hash(value)

    # def generate_password_hash(self, original_password):
    #     """对密码进行加密"""
    #     self.password_hash = generate_password_hash(original_password)

    def check_password(self, pwd):
        """
        检验密码的正确性
        :param pwd: 用户登录输入的原始密码
        :return: 如果正确，返回True， 错误返回False
        """
        # check_password_hash检查错误会返回False
    ...
    def to_full_dict(self):
        ...
        for order in orders:
            comment = {
                ...
                "user_name": order.user.name if order.user.name != order.user.mobile else "匿名用户",  # 显示发表评论的用户，如果用户名不是该用户的手机号，否则显示为“匿名用户”
