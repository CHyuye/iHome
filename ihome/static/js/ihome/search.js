...
var house_data_querying = true;  // 是否正在想后台获取数据
...
// 更新房源列表信息
// action表示从后端请求的数据在前端的展示方式——默认为追加方式
// action=renew 代表页面数据清空从新展示
function updateHouseData(action) {
    var areaId = $(".filter-area>li.active").attr("area-id");
    if (updefined == areaId) areaId = "";
    var startDate = $("#start-date").val();
    var endDate = $("#end-date").val();
    ...
    $.get("/api/v1.0/houses", params, function (resp) {
        ...
        if ("0" == resp.errno) {
            if (0 == resp.data.total_page){
                $(".houses-list").html("暂时没有符合您查询的房屋信息！")
            } else {
...
$(document).ready(function(){
    ...
    $.get("/api/v1.0/areas", function (data) {
        if ("0" == data.errno) {
            // 用户从首页跳转到这个搜素页面时可能选择了城区，所以尝试从url的查询字符串参数中提取用户的选择的城区
            var areaId = queryData["aid"];
            // 如果提取到了城区id的数据
            if (areaId) {
                // 遍历从后盾获取到城区的信息，添加到页面中
                for (var i=0; i<data.data.length; i++) {
                    // 对于宠urk查询字符串参数中拿到的城区，在页面中做高亮展示
                    // 后盾获取到城区id是整型，从url参数中获取到的是字符串类型，所以将uel参数中获取到转换为整型，再进行对比
